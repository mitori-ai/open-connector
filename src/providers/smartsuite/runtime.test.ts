...
describe("SmartSuite compatibility runtime", () => {
  ...
  it("creates a folder only in the replica workspace", async () => {
    const folder = { application: "table-1", solution: "solution-1", label: "Dallas Scheduling", order: 1 };
    const fetchMock = vi.fn(async (request: RequestInfo | URL, init?: RequestInit) => {
      expect(new URL(String(request)).toString()).toBe("https://app.smartsuite.com/api/v1/folders/");
      expect(init?.method).toBe("POST");
      expect(JSON.parse(String(init?.body))).toEqual(folder);
      return Response.json({ id: "folder-1", ...folder });
    });
      ...
      executeSmartsuiteAction(
        {
          ...
          values: { workspaceId: "se4hznb4" },
          actionName: "create_folder",
          input: { tableId: "table-1", solutionId: "solution-1", folder },
        },
        fetchMock as typeof fetch,
      ),
