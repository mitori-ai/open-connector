...
interface OAuthCompletionPageOptions {
  ok?: boolean;
  message?: string;
  returnUrl?: string;
  autoReturn?: boolean;
...
export function renderOAuthCompletionPage(service: string, options: OAuthCompletionPageOptions = {}): string {
  ...
  const status = ok
    ...
    <p>${escapeHtml(options.message ?? "Provider authorization was not completed.")}</p>`;
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>${ok ? "Connection complete" : "OAuth incomplete"}</title>
<style>
:root {
